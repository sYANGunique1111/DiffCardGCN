...
class  MixSTE2_CARD(nn.Module):
    def __init__(self, num_frame=9, num_joints=17, in_chans=2, embed_dim_ratio=32, depth=4,
        ...
        ### spatial patch embedding
        self.Spatial_patch_to_embedding = nn.Linear(in_chans + 6, embed_dim_ratio)
    ...
    def STE_forward(self, x_2d, x_3d, x_3d_hat, t):

        if self.is_train:
            x = torch.cat((x_2d, x_3d_hat, x_3d), dim=-1)
            b, f, n, c = x.shape  ##### b is batch size, f is number of frames, n is number of joints, c is channel size?
            x = rearrange(x, 'b f n c  -> (b f) n c', )
            ### now x is [batch_size, receptive frames, joint_num, 8 channels]
            x = self.Spatial_patch_to_embedding(x)
            # x = rearrange(x, 'bnew c n  -> bnew n c', )
            x += self.Spatial_pos_embed
            time_embed = self.time_mlp(t)[:, None, None, :].repeat(1,f,n,1)
            ...
            # print(type(x_2d),type(x_3d))
            x_2d = x_2d[:,None].repeat(1,x_3d.shape[1],1,1,1)
            # x_3d_hat = x_3d_hat[:,None].repeat(1,x_3d.shape[1],1,1,1)
            x = torch.cat((x_2d, x_3d_hat, x_3d), dim=-1)
            b, h, f, n, c = x.shape  ##### b is batch size, f is number of frames, n is number of joints, c is channel size?
            x = rearrange(x, 'b h f n c  -> (b h f) n c', )
            x = x.type(torch.float32)
            # print(x.dtype, self.Spatial_patch_to_embedding)
            x = self.Spatial_patch_to_embedding(x)
            x += self.Spatial_pos_embed
            time_embed = self.time_mlp(t)[:, None, None, None, :].repeat(1, h, f, n, 1)
            time_embed = rearrange(time_embed, 'b h f n c  -> (b h f) n c', )
    ...
    def forward(self, x_2d, x_3d, x_3d_hat, t):
        if self.is_train:
            b, f, n, c = x_2d.shape
        else:
            b, h, f, n, c = x_3d.shape

        x = self.STE_forward(x_2d, x_3d, x_3d_hat, t)
