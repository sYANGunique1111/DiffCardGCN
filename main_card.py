...
def evaluate(test_generator, best_epoch, action=None, return_predictions=False, use_trajectory_model=False, newmodel=None):
...
def run_evaluation(actions, action_filter=None):
    ...
    for action_key in actions.keys():
        ...
        if args.p2:
            e1, e1_h, e1_mean, e1_select, e2, e2_h, e2_mean, e2_select = evaluate(gen, best_epoch, action_key)
        else:
...
if not args.evaluate:
    ...
    # for test
    # cameras_train = cameras_train[:2]
    # poses_train = poses_train[:2] 
    # poses_train_2d = poses_train_2d[:2]
    ...
    while epoch < args.epochs:
        ...
        for cameras_train, batch_3d, batch_2d in train_generator.next_epoch():
            ...
            # Predict 3D poses
            predicted_3d_pos = model_pos_train(inputs_2d, inputs_3d, inputs_3d)
        ...
        with torch.no_grad():
            ...
            if not args.no_eval:
                ...
                for cam, batch, batch_2d in test_generator.next_epoch():
                    ...
                    predicted_3d_pos = model_pos_test_temp(inputs_2d, inputs_3d, inputs_3d,
...
if args.render:
    ...
    if not args.by_subject:
        run_evaluation(all_actions, best_epoch, action_filter)
    else:
        for subject in all_actions_by_subject.keys():
            print('Evaluating on subject', subject)
            run_evaluation(all_actions_by_subject[subject], best_epoch, action_filter)
            print('')
if not args.nolog:
    writer.close()